...
export default function SignVisibleWatermarkPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/signature/signWithVisibleWatermark`, {
        method: "POST",
        body: formData,
      })
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          <div className="bg-gradient-to-r from-red-500 to-red-600 px-6 py-5 text-white">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-bold">Tanda Tangan Gambar dengan Watermark</h1>
            </div>
            <p className="mt-1 text-red-100">
              Tambahkan watermark yang terlihat pada gambar untuk perlindungan dan identifikasi
            </p>
          </div>
            ...
            <div className="space-y-6">
                ...
                <div className="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:border-red-500 transition-colors">
                  ...
                  <label htmlFor="file" className="cursor-pointer">
                    ...
                    <p className="text-sm text-slate-600 mb-1">
                      {file ? file.name : "Klik untuk memilih gambar atau tarik dan lepas gambar di sini"}
                    </p>
                    <p className="text-xs text-slate-500">
                      {file ? `${(file.size / 1024).toFixed(2)} KB` : "JPG, PNG, dan format gambar lainnya didukung"}
                    </p>
                  </label>
                </div>
              </div>
              ...
              <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-medium text-slate-900">Pengaturan Watermark</h3>
                </div>
