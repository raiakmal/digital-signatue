...
import {
  ...
  FileX,
  QrCode,
  Calendar, // Tambahkan impor icon Calendar
} from "lucide-react";
...
export default function SignFilePage() {
  ...
  // Tambahkan state untuk masa berlaku
  const [validityType, setValidityType] = useState("days");
  const [validityValue, setValidityValue] = useState(7); // Default 7 hari
  ...
  useEffect(() => {
    if (file) {
      ...
      // Clean up function to release the URL when component unmounts or file changes
  ...
  const handleSubmit = async (e) => {
    ...
    // Tambahkan parameter validitas sesuai tipe yang dipilih
    ...
    try {
      ...
      setValidUntil(data.validUntil || ""); // Tambahkan ini untuk menyimpan masa berlaku
    } catch (err) {
      console.error(err);
  ...
  // Sisipkan kode setelah Designer Name Input dan sebelum Submit Button
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 p-3 border border-slate-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
          />
        </div>
        <select
          value={validityType}
          onChange={(e) => setValidityType(e.target.value)}
          className="w-full p-3 border border-slate-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all appearance-none bg-white"
        >
  ...
  // Tambahkan elemen untuk menampilkan masa berlaku di hasil
  const validUntilElement = validUntil && (
    <div>
      ...
      <div className="bg-slate-50 border border-slate-200 p-3 rounded-md text-sm">
        <span className="text-slate-700">{validUntil}</span>
      </div>
    </div>
  );
          ...
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            <div className="p-6">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Tambahkan Validity Input di sini */}
                {validityInputElement}
              ...
              {(signature || hash) && (
                <div className="mt-8 border border-slate-200 rounded-lg overflow-hidden">
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    {/* Tambahkan Valid Until Result di sini */}
                    {validUntilElement}
                    ...
                    {qrCode && (
                      <div>
                        ...
                        <div className="flex justify-center bg-slate-50 border border-slate-200 p-3 rounded-md">
                          <img
                            src={qrCode}
                            alt="QR Code"
          ...
          {file && (
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
              <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-5 text-white">
                ...
                <p className="mt-1 text-blue-100 truncate max-w-md">
                  {file?.name}
                </p>
              </div>

              <div className="p-2 h-[650px] bg-slate-50">
                <FilePreview />
              </div>
            </div>
          )}
